...
async function scrapeUTMScholar() {
  ...
  while (true) {
    ...
    allScholarLinks.push(...scholarLinks); // Add links to the array
    ...
    try {
      // Clicking and handling navigation
      await Promise.all([
        page.evaluate((button) => button.click(), nextButton),
        page.waitForNavigation({ waitUntil: "networkidle2", timeout: 10000 }),
      ]);
  ...
  for (const link of allScholarLinks) {
    try {
      console.log(`Navigating to scholar page: ${link}`);
      await page.goto(link, { waitUntil: "networkidle2", timeout: 60000 }); // Increase timeout to 60 seconds
      ...
      // Continue to the next scholar if error occurs
      continue;
    }
  }
