...
async function scrapeUTMScholar() {
  ...
  try {
    const mainPageUrl = "https://utmscholar.utm.my/faculties/28";
    console.log("Navigating to the UTM Scholar main page...");
    await page.goto(mainPageUrl, { waitUntil: "networkidle0", timeout: 60000 });
    console.log("Main page loaded successfully");

    let pageNumber = 1;
    let allScrapedData = [];
    let previousScholars = new Set();
    ...
    while (true) {
      ...
      // Check if we have new scholars
      const newScholars = scholarLinks.filter(
        (link) => !previousScholars.has(link)
      );
      if (newScholars.length === 0) {
        console.log("No new scholars found. Exiting.");
        break;
      }

      // Process each new scholar link
      for (const link of newScholars) {
        console.log(`Navigating to scholar page: ${link}`);
        await page.goto(link, { waitUntil: "networkidle0", timeout: 300000 });
      ...
      try {
        await Promise.all([
          ...
          page.waitForResponse(
            (response) =>
              response.url().includes("ListScholarFacultyDashboard") &&
              response.status() === 200
          ),
